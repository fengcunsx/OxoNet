...
class EpochPairDataset(Dataset):
    """pos 常驻 RAM; neg 从 pack_train 的 mmap 池按 ctx 轮转采样(每epoch set_epoch重采)。
    pos[i] 与 neg[i] 同 ctx(两侧都按 ctxs 顺序、每ctx取 l_ctx 条) -> MultiSim 配对成立。
    不预生成150文件, 省磁盘; mmap 只读被采行, 省RAM。"""

    def __init__(self, pos_npz, ctx_json, neg_pack_dir):
        super().__init__()
        meta = json.load(open(ctx_json))
        self.ctxs = meta['ctxs']
        self.counts = meta['counts']  # {ctx: l_ctx = 该ctx的pos数}
        p = np.load(pos_npz)
        self.pos_signal = torch.from_numpy(p['signal']).float()
        self.pos_mean = torch.from_numpy(p['mean']).float()
        self.pos_std = torch.from_numpy(p['std']).float()
        self.pos_dwell = torch.from_numpy(p['dwell']).float()
        self.pos_kmer = torch.from_numpy(p['kmers_enc'].astype(np.int64))
        self.pos_readid = p['read_id']
        self.pos_pos = p['basecall_pos']
        self.n = self.pos_signal.shape[0]
        # mmap neg 池 (未压缩 .npy)
        self.neg_mm, self.neg_len = {}, {}
        for ctx in self.ctxs:
            cd = os.path.join(neg_pack_dir, ctx)
            self.neg_mm[ctx] = {f: np.load(os.path.join(cd, f + '.npy'), mmap_mode='r')
                                for f in ['signal', 'mean', 'std', 'dwell', 'kmers_enc']}
            self.neg_len[ctx] = self.neg_mm[ctx]['signal'].shape[0]
        self.set_epoch(0)

    def set_epoch(self, epoch):
        sig, me, st, dw, km = [], [], [], [], []
        for ctx in self.ctxs:
            l, nl = self.counts[ctx], self.neg_len[ctx]
            start = (l * epoch) % nl
            idx = np.arange(start, start + l) % nl  # 顺序轮转(与 generate_pair 一致)
            mm = self.neg_mm[ctx]
            sig.append(np.asarray(mm['signal'][idx]))
            me.append(np.asarray(mm['mean'][idx]))
            st.append(np.asarray(mm['std'][idx]))
            dw.append(np.asarray(mm['dwell'][idx]))
            km.append(np.asarray(mm['kmers_enc'][idx]))
        self.neg_signal = torch.from_numpy(np.concatenate(sig)).float()
        self.neg_mean = torch.from_numpy(np.concatenate(me)).float()
        self.neg_std = torch.from_numpy(np.concatenate(st)).float()
        self.neg_dwell = torch.from_numpy(np.concatenate(dw)).float()
        self.neg_kmer = torch.from_numpy(np.concatenate(km).astype(np.int64))
    ...
    def __getitem__(self, idx):
        real_idx = idx // 2
        if idx % 2 == 0:
            return {"signal": self.pos_signal[real_idx].reshape(-1, 1),
                    "kmer": self.pos_kmer[real_idx],
                    "mean": self.pos_mean[real_idx].reshape(-1, 1),
                    "std": self.pos_std[real_idx].reshape(-1, 1),
                    "dwell": self.pos_dwell[real_idx].reshape(-1, 1),
                    "sig_l": self.pos_dwell[real_idx].sum(),
                    "read_id": self.pos_readid[real_idx], "position": self.pos_pos[real_idx],
                    "label": True}
        return {"signal": self.neg_signal[real_idx].reshape(-1, 1),
                "kmer": self.neg_kmer[real_idx],
                "mean": self.neg_mean[real_idx].reshape(-1, 1),
                "std": self.neg_std[real_idx].reshape(-1, 1),
                "dwell": self.neg_dwell[real_idx].reshape(-1, 1),
                "sig_l": self.neg_dwell[real_idx].sum(),
                "read_id": "", "position": -1,
                "label": False}


def get_dataset_autodl(pos_file, neg_file):
    return PairDataset(pos_file, neg_file)
