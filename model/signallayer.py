...
class SignalConvBlock(nn.Module):
    ...
    @torch._dynamo.disable  # stem 首层 in_channels=1 触发 inductor↔cuDNN 布局bug(2.1/2.4/2.7均有);
    def forward(self, x, mask=None):  # 排除出编译图, 其余照常编译。stem仅~0.8ms, eager跑无损。
...
class RotaryEmbedding(nn.Module):
    """RoPE: 按位置对 Q/K 做旋转, 编码相对位置。不进残差流、无额外参数。
    cos/sin 对固定序列长缓存, 避免每次 forward 重算。"""
    def __init__(self, head_dim: int, base: float = 10000.0):
        super().__init__()
        assert head_dim % 2 == 0, "RoPE 需要偶数 head_dim"
        inv_freq = 1.0 / (base ** (torch.arange(0, head_dim, 2).float() / head_dim))
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self._cache = {}  # (seq_len, device, dtype) -> (cos, sin)
    ...
    def forward(self, seq_len, device, dtype):
        ...
        freqs = torch.outer(t, self.inv_freq.to(device))      # (L, hd/2)
...
class MultiHeadSelfAttention(nn.Module):
    """SDPA 版多头自注意力(等价于 nn.MultiheadAttention 的数学), 支持 pos_mode 开关。
    pos_mode: 'rope'(默认) | 'none' | 'learnable'。RoPE 在每层 attention 对 Q/K 旋转。
    """
    def __init__(self, dim, heads, dropout=0.0, pos_mode="rope", max_len=55):
        super().__init__()
        assert dim % heads == 0
        self.heads = heads
        self.hd = dim // heads
        ...
        if pos_mode == "rope":
            ...
            self.pe = LearnablePositionalEncoding(max_len, dim)

    def forward(self, x, key_padding_mask=None):
        B, L, D = x.shape
        if self.pos_mode == "learnable":
            x = self.pe(x)
        q = self.q_proj(x).view(B, L, self.heads, self.hd).transpose(1, 2)  # (B,H,L,hd)
        k = self.k_proj(x).view(B, L, self.heads, self.hd).transpose(1, 2)
        v = self.v_proj(x).view(B, L, self.heads, self.hd).transpose(1, 2)
        if self.pos_mode == "rope":
            cos, sin = self.rope(L, x.device, x.dtype)
            q, k = _apply_rope(q, k, cos, sin)
        attn_mask = None
        if key_padding_mask is not None:
            # key_padding_mask: (B,L) True=忽略(pad) -> 加性 mask (B,1,1,L)
...
class LongRangeSignal(nn.Module):
    def __init__(self, input_dim, mha_head, dp=0.3, pos_mode="rope"):
        super(LongRangeSignal, self).__init__()
        self.mha = MultiHeadSelfAttention(input_dim, mha_head, dropout=dp, pos_mode=pos_mode)
        self.norm = nn.LayerNorm(input_dim)

    def forward(self, x, mask=None):
        if mask is not None:
            out = self.norm(self.mha(x, key_padding_mask=~mask))
            out = out * mask.unsqueeze(-1).float()
        else:
            out = self.norm(self.mha(x))
        return out
...
class SignalBlock(nn.Module):
    ...
    def __init__(
        ...
        self.global_sig = ResidualConnect(
            LongRangeSignal(input_dim=input_dim, mha_head=mha_head, dp=mha_dp, pos_mode=pos_mode),
        )
