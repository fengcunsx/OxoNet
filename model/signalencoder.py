...
class SignalEncoder(nn.Module):
    ...
    def __init__(self, in_channel=1, out_channel=128, block_count=4, dropout=0.3, sig_l=125):
        super(SignalEncoder, self).__init__()
        self.sig_len = sig_l
        self.conv = SignalConvBlock(in_dim=in_channel, out_dim=out_channel)
        self.blocks = nn.Sequential(
            *[SignalBlock(input_dim=out_channel, mha_dp=dropout, ffn_dp=dropout, conv_dropout_p=dropout) for _ in
              range(block_count)])
