...
def find_best_threshold(df, spec_target=0.999, start=0.5):
    ...
    TP = 0
    FP = 0
    last_t, last_recall, last_spec = start, 0.0, 1.0

    for i in range(len(sorted_probs)):
        if sorted_probs[i] < start:
            break
        ...
        if i + 1 < len(sorted_probs) and sorted_probs[i] == sorted_probs[i + 1]:
            continue

        spec = (total_neg - FP) / (total_neg)
        recall = TP / (total_pos)

        if spec >= spec_target:
            last_t, last_recall, last_spec = sorted_probs[i], recall, spec
        else:
            break  # spec只会继续降，不用再扫了

    return last_t, last_recall, last_spec
...
def prediction(args: argparse.Namespace, resume, generate):
    ...
    if args.resume is not None:
        model.load_state_dict(torch.load(resume, map_location=device)['model_state_dict'], strict=True)
...
def main(args):
    ...
    with torch.no_grad():
        for i in range(50):
            resume = os.path.join(args.resume, 'model_{}.pth'.format(i + 100))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str,
                        default='/home/bio/8oxog/models/3407')
    parser.add_argument('--generate', default=True)
    parser.add_argument('--test-pos-dir', type=str, default='/home/bio/8oxog/data/7mer_feature/valid/8oxog')
    parser.add_argument('--test-neg-dir', type=str, default='/home/bio/8oxog/data/7mer_feature/valid/g')
    ...
    parser.add_argument('--test-work-dir', type=str, default='/home/bio/8oxog/data/workspace')
    parser.add_argument('--batch-size', type=int, default=256)
    parser.add_argument('--threshold', type=float, default=0.5)
    parser.add_argument('--num-workers', type=int, default=8)
    parser.add_argument('--save-dir', type=str, default='/home/bio/8oxog/models/3407/threshold.csv')
